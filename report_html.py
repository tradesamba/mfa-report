...
def build_html(rows, regime_metrics, regime_summary, run_ts):
    ...
    claude_prefix = (
        ...
        "Your role is a quantitative scoring analyst. You are given a structured dataset "
        "produced by an automated Python pipeline (mfa_layer0.py) that fetched prices and "
        "technicals from yfinance, FRED, and Finnhub. The data has already passed a "
        "deterministic integrity gate. Your job is to apply the MFA V6 scoring framework "
        "to this dataset — not to re-fetch or re-verify the numbers. If any figure looks "
        "anomalous to you, apply STAND DOWN for that ticker and note the anomaly; that is "
        "the correct and safe outcome. The scoring framework is designed to be conservative.\n\n"
        f"════ REGIME ════\n{regime_summary}\n\n"
        "════ DATASET (produced by mfa_layer0.py — score as provided) ════\n"
        f"{sec_m}\n\n{sec_b}\n\n{sec_c}\n\n"
        f"{sec_opt}\n\n"
        "════ SENTIMENT SCORES (from Grok Step 1) ════\n")

    claude_suffix = (
        "\n\n════ SCORING INSTRUCTIONS ════\n"
        "Apply the MFA V6 Layer 2 scoring process from the attached guide:\n"
        "1. Apply regime-adjusted category weights. "
        "If Section B fields are N/A, renormalize across Tech + Sentiment only and say so.\n"
        "2. Score each ticker across the 3 categories with intra-category weights. "
        "Never estimate a missing input — treat it as absent.\n"
        "3. Compute weighted confluence (raw score, then scaled ×4 to the −20..+20 range).\n"
        "4. State the regime-adjusted threshold and compare each ticker against it.\n"
        "5. Run Vetoes V1–V7. V2 is direction-adjusted for longs. V7 is exempt for long plays.\n"
        "6. Run the 8-point Pre-Entry Checklist on survivors, including the extension/anti-chase check.\n"
        "7. Rank survivors; if any score changes on closer review, re-rank and repeat until stable.\n"
        "8. Select up to TOP 4: only tickers that pass RVOL gate + zero vetoes + threshold + checklist. "
        "Output fewer if fewer qualify. Output STAND DOWN if none qualify.\n"
        "9. For each top pick, reference the SECTION OPT call data above for the options play. "
        "Use those strikes and ask prices as provided — if a ticker has no OPT data, "
        "give the stock play only.\n"
        "10. Honesty: present confluence scores, not win probabilities. "
        "Confluence ≠ guaranteed outcome. Flag any figure that looks anomalous.\n\n"
        "OUTPUT for each top pick:\n"
        "  (a) STOCK PLAY: entry area, initial stop, price target, brief rationale\n"
        "  (b) OPTIONS PLAY: strike → expiry from SECTION OPT, ask ~$X, why this strike\n"
        "  (c) Confluence score breakdown + honest assessment of setup quality\n"
        "+ summary table of all scored tickers. Or STAND DOWN with reasons.")
